...
let run_rg (args : string array) : (string list, process_error) result =
  ...
  | exception Unix.Unix_error (err, _, _) -> Error (Launch_failed (Unix.error_message err))
  | channel -> begin
      ...
      | out -> begin
          ...
          | Unix.WEXITED 0 ->
              String.concat "\n" out |> Printf.eprintf "Ripgrep output:\n%s\n";
              Ok out
          | Unix.WEXITED 1 -> Ok []
...
let parse_rg_location (line : string) : int * Todo.location =
  try
    ...
    let result : Todo.location = {
        ...
        line   = int_of_string
                  (String.sub line (i_null + 1) (i_line_end - i_null - 1));
        column = int_of_string
                  (String.sub line (i_line_end + 1) (i_col_end - i_line_end - 1));
    } in
    let read = i_col_end in (read, result)
  with
