...
let update_readme_lines ?(file_path = default_file_path)
  ...
  let tmp_readme = Filename.temp_file (Filename.remove_extension filename) ext
  in
  ...
  Fun.protect begin fun () ->
    ...
    let process_lines ic oc =
      let rec loop () = match In_channel.input_line ic with
        | Some line ->
    ...
    let* oc = try Ok (Out_channel.open_text tmp_readme)
      with Sys_error msg -> Error (Write_error msg)
    in outch := Some oc;

    let* file_exists = try Ok (Sys.file_exists file_path)
      with Sys_error msg -> Error (Read_error msg) in

    let* () = match file_exists with
      | true ->
          let* ic = try Ok (In_channel.open_text file_path)
