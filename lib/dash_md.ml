...
let footer = "<sub>_— Generated by [MuadʾDebt](https://github.com/cz4rny/muaddebt) 🐫_</sub>"
...
> Where the debt has gone, only flow remains.@.|};
  ...
  let open Todo in
  let most_urgent_first_todos = todos |> List.stable_sort (fun t1 t2 ->
  ...
  in
  most_urgent_first_todos |> List.iter (fun t ->
    let loc = t.location in
    fprintf fmt "@.  - [%s: %s](%s#L%d)" (Marker.to_string t.marker)
